...
export class Grafana_Prometheus_Quickwit{

    //Class Variables/Properties:
    //config: Easy_EKS_Config_Data; is purposefully omited, to avoid confusing human users with multiple instances of object sharing different state
    //passing config as input in function, keeps it to 1 copy.
    stack: cdk.Stack;
    ...
    constructor(stack: cdk.Stack, cluster: eks.ICluster){
        ...
        this.observability_ns = new eks.KubernetesManifest(stack, "monitoring-kube-namespace", {
            cluster: cluster,
            manifest: [this.observability_ns_manifest],
            overwrite: true,
            prune: true,
        });
    }//end constructor of Grafana_Prometheus_Quickwit
...
export function enable_metrics_observability_via_quickwit(config: Easy_EKS_Config_Data, stack: cdk.Stack, cluster: eks.ICluster, input: EKS_Metrics_via_GPQ_Input_Parameters){
...
export function enable_logs_observability_via_quickwit(config: Easy_EKS_Config_Data, stack: cdk.Stack, cluster: eks.ICluster, input: EKS_Logs_via_GPQ_Input_Parameters){
