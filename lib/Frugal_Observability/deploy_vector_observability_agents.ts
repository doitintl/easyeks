...
- name: IP_OF_HOST_WORKER_NODE  #<-- Note, this returns an IPv6 IP, which needs an IPv6 specific syntax http://[$IP_OF_HOST_WORKER_NODE]:9100/metrics
  valueFrom:
    fieldRef:
      fieldPath: status.hostIP  #<-- downward API to learn host node's IP, for curling prometheus-node-exporter, which uses hostNetwork: true
containerPorts:
...
customConfig:
  ...
  sources:
    kube_worker_node_vector_host_metrics:
      type: host_metrics
      namespace: kube_worker_node_vector_host_metrics   #(<-- metric namespaces allow vector aggregator to route subsets of metrics)
      filesystem:
        devices:
          excludes:
          - 'binfmt_misc'
    ...
    kube_worker_node_prometheus_node_exporter_metrics:
      type: prometheus_scrape
      endpoints:
      - http://[\$IP_OF_HOST_WORKER_NODE]:9100/metrics   #<-- The prometheus-node-exporter daemonset uses host_network_true & env var returns IPv6
...
#    victoria_metrics_db_metrics:
#      type: prometheus_scrape
#      endpoints:
#      - http://WIP:9100/metrics
#      scrape_interval_secs: 15
#      scrape_timeout_secs: 10
...
#      - vector_aggregator_generated_test_metrics
      route: #v-- Pattern is log_name: 'VRLcondition' (https://vector.dev/docs/reference/vrl/#example-filtering-events)
        ...
        kube_worker_node_vector_host_metrics: '.namespace == "kube_worker_node_vector_host_metrics"'                           #<-- now "subset_of_metrics.kube_worker_node_vector_host_metrics" is a recognized input
        kube_worker_node_prometheus_node_exporter_metrics: '.namespace == "kube_worker_node_prometheus_node_exporter_metrics"' #<-- now "subset_of_metrics.kube_worker_node_prometheus_node_exporter_metrics" is a recognized input
    subset_of_logs:
      type: "route"
      reroute_unmatched: false
      inputs: 
...
#        # ^-- Thanks to this "log_source: vector_generated_test_logs" becomes a working query for Victoria Logs
  sinks: #<--(sink = destination)
    prometheus_exporter:          #<-- You can test using kubectl port-forward -n=observability svc/observability-aggregator-vector 9090:9090
      ...
      inputs:
      - subset_of_metrics.kube_worker_node_vector_host_metrics
#      - subset_of_metrics.kube_worker_node_prometheus_node_exporter_metrics
